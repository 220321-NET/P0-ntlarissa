...
public class ManagerController : ControllerBase
{
    ...
    public ManagerController(IStoreBL bl, IMemoryCache cache)
    {
        _bl = bl;
        _cache = cache;
    }
    ...
    {
        User? gotUser = await _bl.getManagerAsync(username);
        if (gotUser != null)
        {
            return Ok(gotUser);
        }
    ...
    {
        User? createdUser = _bl.createNewManager(userToCreate);
        if (createdUser != null)
        {
